...
static void ofputil_match_to_ofp15(const struct match *, struct match *);
static void ofputil_ofp15_to_match(const struct match *, struct match *);
                          ...
                          struct ofpbuf *ofpacts)
{
    ...
    if (raw == OFPRAW_OFPT15_PACKET_OUT) {
        ...
        ofputil_ofp15_to_match(&match, &(po->flow_metadata));
...
static void
ofputil_ofp15_to_match(const struct match *match,
                     struct match *fmd)
{
    ...
    fmd->flow.nw_src = match->flow.nw_src;
    fmd->flow.nw_dst = match->flow.nw_dst;
    fmd->flow.nw_tos = match->flow.nw_tos;
...
/* Returns the OpenFlow 1.5+ match fields equivalent to the OpenFlow 1.0 match fields
 * , for encoding OpenFlow 1.5+ protocol messages.
 *
 * See the definition of OFP11_MAX for an explanation of the mapping. */
static void
ofputil_match_to_ofp15(const struct match *util_match,struct match *ofp15_match)
{
    ...
    */
    ...
    if (!eth_addr_is_zero(util_match->flow.dl_dst)) 
    {
        match_set_dl_dst(ofp15_match, util_match->flow.dl_dst);
    }
